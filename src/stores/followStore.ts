...
import { getFollowers, getFollowing, addFollow, removeFollow } from '../services/socialService';
...
interface FollowState {
  ...
  removeFollowing: (currentUserId: string, userIdToUnfollow: string) => Promise<void>;
  reset: () => void;
}
    ...
    (set, get) => ({
      ...
      loadFollowers: async (userId: string) => {
        if (!get().hasLoadedFollowers || get().followers.length === 0) {
          ...
          const followersData = await Promise.all(followerIds.map(async id => {
            const existingFollower = get().followers.find(f => f.email === id);
            if (existingFollower) return existingFollower;
            const userData = await getUser(id);
            return userData ? transformUser(userData, get().following.some(f => f.email === id)) : null;
          }));
          set({ 
      ...
      loadFollowing: async (userId: string) => {
        if (!get().hasLoadedFollowing || get().following.length === 0) {
          ...
          const followingData = await Promise.all(followingIds.map(async id => {
            const existingFollowing = get().following.find(f => f.email === id);
      ...
      addFollower: (user: User) => set(state => ({
        followers: [...state.followers, transformUser(user, false)]
      })),

      removeFollower: (userId: string) => set(state => ({
        followers: state.followers.filter(follower => follower.email !== userId)
      })),
      ...
      removeFollowing: async (currentUserId: string, userIdToUnfollow: string) => {
        try {
          await removeFollow(currentUserId, userIdToUnfollow);
          set(state => ({
            following: state.following.filter(user => user.email !== userIdToUnfollow),
            followers: state.followers.map(follower => 
