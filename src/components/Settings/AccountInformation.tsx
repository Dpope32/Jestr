...
const AccountInfoItem: React.FC<AccountInfoItemProps> = ({ label, value, onUpdate }) => {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
      ...
      <TouchableOpacity onPress={isEditing ? handleSend : handleEdit}>
        <FontAwesomeIcon icon={isEditing ? faPaperPlane : faPencilAlt} size={18} color="#1bd40b" />
      </TouchableOpacity>
    </View>
  );
};
...
const AccountInformation: React.FC<AccountInformationProps> = ({ closeModal }) => {
  ...
  return (
    <View style={styles.container}>
      <AccountInfoItem label="Username" value={username} onUpdate={handleUsernameUpdate} />
      <AccountInfoItem label="Email" value={email} onUpdate={handleEmailUpdate} />
      <AccountInfoItem label="Display Name" value={displayName} onUpdate={handleDisplayNameUpdate} />
    </View>
  );
};
