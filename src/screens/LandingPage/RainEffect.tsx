...
const RainEffect = () => {
  ...
  const animatedValues = useRef([...Array(numDrops)].map(() => new Animated.Value(0))).current;
  const animationRefs = useRef<Animated.CompositeAnimation[]>([]);

  useEffect(() => {
    animationRefs.current = animatedValues.map((animatedValue, index) => {
      const delay = Math.random() * 2000; // Random delay for each raindrop
