...
const Inbox: React.FC = () => {
  ...
  const filteredConversations = useMemo(() => 
    conversations.filter((conv) => !pinnedConversations.some((pinnedConv) => pinnedConv.id === conv.id)),
        ...
        if (isMemeShareContent(parsedContent)) {
          ...
          return (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Image
                source={{ uri: memeUrl }}
                style={{ width: 40, height: 40, borderRadius: 5,  marginTop: -20, marginLeft: 20}}
              />
            </View>
          );
        }
  ...
  const renderItem = useCallback(({ item }: { item: Conversation }) => {
    ...
    return (
      ...
      >
        <Image
          source={{
            uri: item.partnerUser.profilePic?.trim() || DEFAULT_PROFILE_PIC_URL,
          }}
  ...
  }, [pinnedConversations, unpinConversation, pinConversation, deleteConversation]);
  ...
  }, [isStoreLoading, isServerLoading, error, pinnedConversations, filteredConversations, renderItem, renderHiddenItem]);
      ...
      <View style={styles.header}>
        ...
        {user && (
          <TouchableOpacity onPress={() => handleProfileClick(drawerNavigation)}>
            <Image
              source={{uri: typeof user.profilePic === 'string' && user.profilePic.trim() !== '' ? user.profilePic.trim(): DEFAULT_PROFILE_PIC_URL }}
              style={styles.profilePic}
            />
          </TouchableOpacity>
        )}
