...
export const useMemes = (user: User | null, accessToken: string | null) => {
    ...
    debounce(
      async (
        ...
        try {
          ...
          setMemes(prevMemes =>
            lastViewedMemeId ? [...prevMemes, ...result.memes] : result.memes,
          );
          setLastEvaluatedKey(result.lastEvaluatedKey);
          setAllMemesViewed(
            result.memes.length === 0 || !result.lastEvaluatedKey,
          );
        } catch (error) {
  ...
  const fetchInitialMemes = useCallback(async () => {
    if (!user?.email || !accessToken || isLoading || !(await acquireLock()))
      return;
    try {
      setIsLoading(true);
