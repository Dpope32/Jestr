...
const checkBadgeEligibility = async (userEmail, action) => {
  ...
  try {
    ...
    console.log(`User ${userEmail} has a count of ${count} for action '${action}'`);
...
const awardBadge = async (userEmail, badgeType) => {
  try {
    console.log(`Starting to award badge '${badgeType}' to user '${userEmail}'`);
    
    // Check if badge already exists
    const existingBadges = await getUserBadges(userEmail);
    console.log(`Existing badges for ${userEmail}:`, existingBadges);
    
    if (existingBadges.some((badge) => badge.BadgeType === badgeType)) {
      console.log(`User ${userEmail} already has badge '${badgeType}'`);
      return;
    }
...
const getUserBadges = async (userEmail) => {
  ...
  console.log(`Params for getUserBadges:`, params);
