...
const updateUserProfile = async (
  ...
  const updateParams = {
    ...
    Key: {email: email},
    UpdateExpression:
      'set username = :username, profilePic = :profilePic, displayName = :displayName, bio = :bio, headerPic = :headerPic, LastLogin = :lastLogin, accessToken = :accessToken',
    ...
    switch (operation) {
      ...
      case 'fetchMemes': {
        ...
        try {
          ...
          const userViewsResult = await docClient.send(
            new QueryCommand(userViewsParams),
          );
          const seenMemeIDs = new Set(
            userViewsResult.Items.map(item => item.MemeID),
          );

          let unseenMemes = [];
          let lastEvaluatedKeyForMemes = lastEvaluatedKey
          ...
          while (unseenMemes.length < limit && attempts < maxAttempts) {
            ...
            const newUnseenMemes = result.Items.filter(
              meme => !seenMemeIDs.has(meme.MemeID),
            );
          ...
          return createResponse(200, 'Memes retrieved successfully.', {
            memes: memes,
            lastEvaluatedKey: lastEvaluatedKeyForMemes
              ? JSON.stringify(lastEvaluatedKeyForMemes)
              : null,
          });
        } catch (error) {
      ...
      case 'getUser': {
        ...
        if (!identifier) {
          ...
          return createResponse(
            400,
            'Identifier is required for getting user details.',
          );
        }
        try {
      ...
      case 'updateBio': {
        ...
        if (!email || bio === undefined) {
          return createResponse(
            400,
            'Email and bio are required for updating bio.',
          );
        }
        ...
        try {
          ...
          return createResponse(200, 'Bio updated successfully.', {
            data: {updatedBio: bio},
          });
        } catch (error) {
      ...
      case 'getConversations': {
        ...
        try {
          ...
          const primaryConversations = await docClient.send(
            new QueryCommand(primaryConversationsParams),
          );
          ...
          const secondaryConversationsParams = {
            TableName: 'UserConversations',
            FilterExpression:
              'contains(ConversationID, :userID) AND NOT begins_with(UserID, :userID)',
            ExpressionAttributeValues: {
              ':userID': userID,
            },
          };
          const secondaryConversations = await docClient.send(
            new ScanCommand(secondaryConversationsParams),
          );

          // Combine both primary and secondary conversations
          const allConversationsItems = [
            ...primaryConversations.Items,
            ...secondaryConversations.Items,
          ];
          ...
          const conversationsWithDetails = await Promise.all(
            uniqueConversations.map(async item => {
              ...
              const {Items: messages} = await docClient.send(
                new QueryCommand(messageParams),
              );
              const lastMessage =
                messages.length > 0
                  ? messages[0]
              ...
              const partnerUserID = conversationID
                .split('#')
                .find(id => id !== userID);
              const userParams = {
                TableName: 'Profiles',
              ...
              const profileResponse = await docClient.send(
                new GetCommand(userParams),
              );
              const partnerUserDetails = profileResponse.Item || {
                username: null,
                profilePic: null,
              };
      ...
      case 'updateProfileImage': {
        ...
        if (!email || !imageType || !image) {
          ...
          return createResponse(
            400,
            'Email, imageType, and image are required for updating profile image.',
          );
        }
        ...
        try {
          ...
          if (imageType === 'profile') {
            ...
            newImageUrl =
              'https://jestr-bucket.s3.amazonaws.com/HeaderPictures/${headerPicKey}';
          } else {
          ...
          const updateProfileResult = await docClient.send(
            new UpdateCommand(updateProfileParams),
          );
          ...
          if (imageType === 'profile') {
            ...
            const memesResult = await docClient.send(
              new QueryCommand(queryMemesParams),
            );
          ...
          return createResponse(200, 'Profile image updated successfully.', {
            [imageType + 'Pic']: newImageUrl,
          });
        } catch (error) {
      ...
      case 'shareMeme': {
        ...
        if (!memeID || !email || !username || !catchUser) {
          return createResponse(
            400,
            'MemeID, email, username, and catchUser are required for sharing a meme.',
          );
        }
        try {
          ...
          return createResponse(
            200,
            'Meme shared successfully and message sent.',
          );
        } catch (error) {
      ...
      case 'batchCheckStatus': {
        try {
          ...
          if (!memeIDs || !Array.isArray(memeIDs) || memeIDs.length === 0) {
            return createResponse(
              400,
              'memeIDs is required and must be a non-empty array.',
            );
          }
          if (!userEmail) {
          ...
          if (
            !followeeIDs ||
            !Array.isArray(followeeIDs) ||
            followeeIDs.length === 0
          ) {
            return createResponse(
              400,
              'followeeIDs is required and must be a non-empty array.',
            );
          }

          console.log('Processing batch status check:', {
            userEmail,
            followeeIDs,
          });
      ...
      case 'completeProfile': {
        ...
        const {email, username, profilePic, headerPic, displayName, bio} =
          requestBody;
        ...
        if (!email || !username) {
          return createResponse(
            400,
            'Email and username are required for profile completion.',
          );
        }
        ...
        try {
          ...
          let profilePicUrl =
            'https://jestr-bucket.s3.us-east-2.amazonaws.com/HeaderPictures/b%40b-headerPic.jpg';
          let headerPicUrl =
            'https://jestr-bucket.s3.us-east-2.amazonaws.com/ProfilePictures/default-profile-pic.jpg';
      ...
      case 'recordMemeView':
        try {
          ...
          const batchWriteParams = {
            RequestItems: {
              UserViewHistory: Object.values(uniqueMemeViews).map(
                (view, index) => ({
                  PutRequest: {
                    Item: {
                      email: {S: view.email},
                      ...
                      Timestamp: {
                        S: new Date(Date.now() + index).toISOString(),
                      },
          ...
          console.log(
            'BatchWrite params:',
            JSON.stringify(batchWriteParams, null, 2),
          );
