...
import Redis from 'ioredis'; // Changed to ES module import
...
/**
 * Function to get the share count for a user.
 * @param {string} email - User's email.
 * @returns {Promise<number>} - Number of shares.
 */
async function getShareCountForUser(email) {
  const params = {
    TableName: 'Shares',
    IndexName: 'UserEmail-index', // Assuming you have a GSI on UserEmail
    ...
    switch (operation) {
      ...
      case 'shareMeme': {
        ...
        try {
          ...
          const shareCount = await getShareCountForUser(email);
          let badgeEarned = null;
          if (shareCount === 25) {
            badgeEarned = await awardBadge(email, 'viralSensation'); 
          }
          
          return createResponse(200, 'Meme shared successfully and message sent.', { badgeEarned });
        } catch (error) {
